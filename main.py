...
for i in range(len(lines)):
    if "facet normal" in lines[i]:
        ...
        normal_data = [float(normal_data_str[k].split("\n")[0]) for k in range(2, 5)]
        ...
        for j in range(0,3):
            ...
            for k in range(1,4):
                vertex_data.append(float(vertex_data_str[k].split("\n")[0]))
...
#F vector = xi+yj+zk
#volume = summation(tess1stpt.(normal * Area))/6.0
...
print(f"Area of the stl file is :{total_area}")
